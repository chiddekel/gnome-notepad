...
public:
   ...
   * which varies by locale. Both this date and `first_day_of_week` must be valid.
   *
   * If this date is before the start of the first week of the year (for example,
   * before the first Monday in January if @c first_day_of_week is
   * Glib::Date::Weekday::MONDAY) then zero will be returned.
   *
   * @param first_day_of_week The day which is considered the first day of the week.
   *    (For example, this would be Glib::Date::Weekday::SUNDAY in US locales,
   *    Glib::Date::Weekday::MONDAY in British locales, and
   *    Glib::Date::Weekday::SATURDAY in Egyptian locales.)
   * @return Week number (starting from 1), or `0` if this date is before the start
   *    of the first week of the year.
   *
   * @newin{2,86}
   */
  unsigned int get_week_of_year(Weekday first_day_of_week) const;
   ...
   * function effectively calculates how many @c first_day_of_week days there are in
   * the year.
   *
   * @param year Year to count weeks in.
   * @param first_day_of_week The day which is considered the first day of the week.
   ...
   *    Glib::Date::Weekday::MONDAY in British locales, and
   *    Glib::Date::Weekday::SATURDAY in Egyptian locales.)
   * @return The number of weeks in @c year.
   * @newin{2,86}
   */
  static guint8 get_weeks_in_year(Year year, Weekday first_day_of_week);
