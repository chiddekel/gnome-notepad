...
Glib::RefPtr<Font> FontMap::reload_font(const Glib::RefPtr<Font>& font, double scale, const Glib::RefPtr<Context>& context, const Glib::ustring& variations) const
{
  return Glib::wrap(pango_font_map_reload_font(const_cast<PangoFontMap*>(gobj()), Glib::unwrap(font), scale, Glib::unwrap(context), variations.empty() ? nullptr : variations.c_str()));
}

bool FontMap::add_font_file(const std::string& filename)
{
  GError* gerror = nullptr;
  auto retvalue = pango_font_map_add_font_file(gobj(), filename.c_str(), &(gerror));
  if(gerror)
...
Glib::PropertyProxy_ReadOnly< GType > FontMap::property_item_type() const
{
  return Glib::PropertyProxy_ReadOnly< GType >(this, "item-type");
}

Glib::PropertyProxy_ReadOnly< unsigned int > FontMap::property_n_items() const
{
